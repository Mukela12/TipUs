...
export default function EmployeeTipsPage() {
  const { tips, tipStats, profile, fetchVenueTips } = useEmployeeDashboardStore()
  ...
  }, [profile?.venue_id, fetchVenueTips, statusFilter, dateFrom, dateTo])
  ...
  const statCards = [
    { label: 'Total Tips', value: formatCurrency(tipStats.totalTips), icon: DollarSign, color: 'bg-primary-100 text-primary-600' },
    { label: 'Average Tip', value: formatCurrency(tipStats.avgTip), icon: BarChart3, color: 'bg-accent-100 text-accent-600' },
    { label: 'This Week', value: formatCurrency(tipStats.tipsThisWeek), icon: TrendingUp, color: 'bg-success-light text-success' },
    { label: 'Tip Count', value: tipStats.tipCount, icon: Hash, color: 'bg-info-light text-info' },
  ]
  ...
  return (
    <div>
      ...
      <motion.div variants={fadeInUp} initial="hidden" animate="visible">
        <h1 className="text-xl font-bold text-surface-900 sm:text-2xl">Venue Tips</h1>
        <p className="mt-0.5 text-sm text-surface-500">
          All tips received by your venue.
        </p>
      </motion.div>
                ...
                {tips.map((tip) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm text-surface-500 max-w-[200px] truncate">
                      {tip.tipper_message || <span className="text-surface-300">&mdash;</span>}
                    </td>
                  </motion.tr>
                ))}
              </tbody>
